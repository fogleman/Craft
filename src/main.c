    ...
    while (running) {
        ...
        while (1) {
            ...
            if (SHOW_INFO_TEXT) {
                ...
                snprintf(coordinates, 1024, "coordinates part 2: x:%.2f y:%.2f z:%.2f", s->x, s->y, s->z); //newline test
                render_text(&text_attrib, ALIGN_LEFT, tx, ty - (4 * ts), ts, coordinates); //newline test
